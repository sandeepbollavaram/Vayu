...
- **M3.2** — first real connector: Gemini. `Vayu.AI.Gemini.GeminiProvider` implements `IOnlineAiProvider`. The key is resolved through `SecureConfigService` (Credential Manager → env → DPAPI) and stays **internal to the assembly** — `GetKeyStatusAsync` returns only presence + source, never the value. `PlanAsync` makes **no HTTP call** unless consent is `AllowOnce` and a key is configured; output is allowlist/risk-validated. Tested with a fake `HttpMessageHandler`; no real key, no real network.
- **M3.3** — secure key-setup UI. `GeminiKeySetupService` saves a pasted key to Windows Credential Manager (DPAPI-encrypted-config fallback) from the Settings "Online AI · Gemini" card. The textbox clears on save, the key is never shown again, and the status exposes only Configured/Not configured + source. Env-var keys are honoured but not removable from Vayu. The same shape applies to every other registry provider's future key-setup.
- **M3.4** — cloud consent dialog. `DesktopCloudConsentService` (WinUI) implements `ICloudConsentService`: every cloud call is gated by an **Allow once / Use local instead / Cancel** dialog (dismiss = Cancel) that names the provider, purpose, data summary, estimated prompt size, and sensitive-context flag. The Settings "Test key" button now works behind this gate — a minimal redacted Gemini round-trip that fires only on Allow once, never logging the prompt body or key. Consent is not persisted; every provider call asks again.
- **M3.5** — online/hybrid router integration. `AiRouterIntentPlanner` gains `PlanningMode` (RuleBased / Offline / Online / Hybrid). Online and Hybrid run `GeminiProvider.PlanAsync` behind the consent dialog (Allow once → cloud; Use local instead / Cancel → no cloud), asking consent at most once per command and always falling back to the rule-based parser on any decline/failure. Settings AI Mode is a readiness-gated 4-option selector. Plans are allowlist/risk-validated and still flow through the permission engine + audit log. Other providers reuse this exact router shape — only the connector changes.
- **M3.6** — Provider Registry UI. The Settings "Online Provider Registry" card surfaces all catalog entries (display name, id, kind, capability chips, status) via the pure `ProviderRegistryViewModel`. **Gemini** is the one provider marked "Available now"; the rest are "Planned" with disabled actions, so the multi-provider story is visible without implying unsupported providers work. No card carries a key field; no non-Gemini cloud call is possible from this UI.
- **M3.7** — connector shells. `PlannedOnlineProvider` is a single reusable `IOnlineAiProvider` shell; `PlannedOnlineProviders` builds one for every non-Gemini catalog entry, with dedicated shells for OpenAI / Claude / DeepSeek / Kimi / OpenRouter / Custom OpenAI-compatible. Each shell's `PlanAsync` returns a safe "planned, not implemented yet" failure — **no HTTP, no key read, no consent, no execution**. The registry shows these as "Planned · connector shell present". Gemini remains the **only** working connector through the M3 release; future milestones flip one real connector on at a time behind the same `IOnlineAiProvider` + consent + `OnlineAiSafetyPolicy` contract.
