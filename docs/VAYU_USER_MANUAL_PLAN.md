...
- Vayu **runs from your chosen storage root**: logs, cache, models, and generated assets live under it. Verify the active paths any time in Settings → Storage (Open data folder). Changing the root takes effect next launch; existing data is not moved (migration is a future feature). The small settings database stays at the default location so Vayu always knows where your root is.
