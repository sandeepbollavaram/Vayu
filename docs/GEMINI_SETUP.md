...
> **Status (M3 complete):** Gemini is the **only fully working** online connector. The connector (M3.2), secure key-setup UI (M3.3), consent dialog (M3.4), Online/Hybrid router (M3.5), provider registry (M3.6), and connector shells for the other providers (M3.7) are all in place. In **Settings → Online AI · Gemini** you save a key (Credential Manager / DPAPI fallback — cleared on save, never shown again); in **Settings → AI Mode** you pick **Online Gemini** or **Hybrid**. Every cloud planning call asks for consent (**Allow once / Use local instead / Cancel**, dismiss = Cancel), at most once per command; **Use local instead** and **Cancel** make no cloud call. The plan still flows through `AgentRuntime → PermissionService → audit`. Cloud AI is **off by default** (default mode is Rule-based); no prompt body or key is ever logged. The other registry providers (OpenAI/Claude/DeepSeek/Kimi/OpenRouter/Custom) are **planned connector shells** — no real calls yet. See [M3_DEMO.md](M3_DEMO.md) for the full walkthrough.
