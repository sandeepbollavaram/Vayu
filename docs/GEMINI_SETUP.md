...
> **Status:** as of **M3.6** the Settings **Online Provider Registry** shows every catalog provider — Gemini marked "Available now", the rest "Planned · Coming in M3.7+". The full Online/Hybrid planning path (M3.5) is live. The Gemini connector (M3.2), secure key-setup UI (M3.3), and cloud consent dialog (M3.4) feed the **AI Router**: in **Settings → AI Mode** you can pick **Online Gemini** or **Hybrid** once a key is saved. Every cloud planning call asks for consent (**Allow once / Use local instead / Cancel**, dismiss = Cancel), at most once per command; **Use local instead** and **Cancel** make no cloud call. The resulting plan still flows through `AgentRuntime → PermissionService → audit`. Cloud AI remains **off by default** (the default mode is Rule-based), and no prompt body or key is ever logged. Provider registry UI polish is **M3.6**; OpenAI/Claude/DeepSeek/Kimi shells are **M3.7**.
