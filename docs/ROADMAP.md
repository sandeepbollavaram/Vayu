...
| M4.13 | ✅ **Storage paths drive runtime** — `VayuStoragePathProvider` resolves the active storage paths at startup (saved root if valid, else safe defaults; creates folders; fail-safe). Logs write under the active logs folder and Whisper downloads default to the active models folder; cache/assets exposed. Setup gains a folder **Browse…** picker + derived-path preview; Settings gains a **Storage** card (active paths, Open data folder, CUSTOM/DEFAULT). Bootstrap settings DB stays at default; no silent data migration (documented future). |
