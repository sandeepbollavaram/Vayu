...
| M2.3 | Local model catalog wiring — Settings reads `LocalModelCatalog`; UI shows recommended/fallback/alternative. |
| M2.4 | Installed model listing — `ListLocalModelsAsync` parses Ollama's `/api/tags` and reconciles with the curated catalog. |
